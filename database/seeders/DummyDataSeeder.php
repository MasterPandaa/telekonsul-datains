...
class DummyDataSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            if ($existingUser) {
                $dokterUsers[] = $existingUser;
                
                $existingDokter = Dokter::where('user_id', $existingUser->id)->first();
                if ($existingDokter) {
                    $dokters[] = $existingDokter;
                }
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            $existingUser = User::where('email', $email)->first();
            if ($existingUser) {
                $pasienUsers[] = $existingUser;
                
                $existingPasien = Pasien::where('user_id', $existingUser->id)->first();
                if ($existingPasien) {
                    $pasiens[] = $existingPasien;
                }
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            $existingUser = User::where('email', $email)->first();
            if ($existingUser) {
                $dosenUsers[] = $existingUser;
                
                $existingDosen = Dosen::where('user_id', $existingUser->id)->first();
                if ($existingDosen) {
        ...
        // Buat data konsultasi hanya jika belum ada
        if (Konsultasi::count() == 0) {
            ...
            foreach ($dokterUsers as $dokterUser) {
                foreach ($pasiens as $pasien) {
                    ...
                    for ($i = 1; $i <= 5; $i++) {
                        ...
                        if ($i <= 3 && count($dosens) > 0) { // 60% of completed konsultasi have dosen evaluations
            ...
            $this->command->info("Skipping konsultasi creation as data already exists.");
        }
    }
    ...
    {
        // Tidak menghapus data yang ada, hanya untuk memastikan tidak ada duplikasi
        // Jika ingin menghapus data, uncomment kode di bawah ini
        /*
