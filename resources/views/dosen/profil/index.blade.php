        ...
        <div class="p-6 flex flex-col items-center text-center space-y-4">
            <div class="relative">
                <div class="w-40 h-40 rounded-full overflow-hidden ring-4 ring-blue-50 shadow-sm bg-white">
                    <img src="{{ $fotoUrl }}" alt="{{ $displayName }}" class="w-full h-full object-cover cursor-pointer" onclick="openFotoModal(this.src)">
                </div>
            </div>
            <div class="space-y-2">
            ...
            <div class="w-full pt-4 border-t border-gray-100">
                <form action="{{ route('profile-photo.update') }}" method="POST" enctype="multipart/form-data" class="w-full">
                    @csrf
                    <input type="file" name="foto" accept="image/*" class="hidden" id="dosen-profile-photo-input" onchange="profilePhotoConfirmAndSubmit(this)">
                    <button type="button" onclick="document.getElementById('dosen-profile-photo-input').click()" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-md text-sm transition">
                        Ubah Foto Profil
                    </button>
                </form>
            </div>
