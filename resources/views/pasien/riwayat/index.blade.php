    ...
    <div class="mb-6">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <a href="{{ route('pasien.konsultasi.create') }}"
                class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-md text-sm transition flex items-center justify-center w-full md:w-auto">
            ...
            <form action="{{ route('pasien.riwayat.index') }}" method="GET" class="space-y-4">
                <div class="flex flex-col lg:flex-row lg:items-center lg:space-x-4 space-y-4 lg:space-y-0">
                    ...
                    <div class="w-full lg:w-64">
                        <div class="relative">
                            <select name="dokter"
                                class="w-full pl-4 pr-12 py-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring focus:ring-blue-200 transition-all duration-200 bg-gray-50 text-gray-800 hover:bg-gray-100 cursor-pointer">
                    ...
                    <div class="w-full lg:w-48">
                        <div class="relative">
                            <select name="period"
                                ...
                                <option value="180" {{ request('period') == '180' ? 'selected' : '' }}>6 Bulan Terakhir
                                </option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="w-full lg:w-auto">
                        <button type="submit"
                            class="w-full lg:w-auto bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-6 rounded-xl transition-all duration-200 flex items-center justify-center space-x-2 shadow-sm hover:shadow transform hover:-translate-y-0.5">
        ...
        @if($riwayatKonsultasi->isEmpty())
            <div class="p-6 text-center">
                <div class="py-8">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Riwayat konsultasi Anda akan muncul di sini setelah konsultasi
                        selesai.</p>
                    <div class="mt-6">
            ...
            <div class="p-0">
                        ...
                        <thead>
                            <tr class="bg-gray-50">
                                ...
                                <th scope="col"
                                    ...
                                    Rating</th>
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-medium text-gray-800 uppercase tracking-wider">Aksi
                                </th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-gray-50 transition" id="konsultasi-{{ $item->id }}">
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <img class="h-10 w-10 rounded-full object-cover bg-gray-100"
                                                    src="{{ $item->dokter->dokter->foto_url ?? asset('img/default.jpg') }}"
                                                    alt="{{ $item->dokter->name ?? 'Dokter' }}">
                                            </div>
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $item->dokter->name ?? '-' }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $item->tanggal->isoFormat('D MMMM Y') }}
                                        </div>
                                        <div class="text-xs text-gray-500 mt-1">{{ substr($item->jam_mulai, 0, 5) }} -
                                            {{ substr($item->jam_selesai, 0, 5) }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        @if($item->rating)
                                            <div class="flex items-center">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <svg class="w-5 h-5 {{ $i <= $item->rating ? 'text-yellow-400' : 'text-gray-300' }}"
                                        ...
                                        @else
                                            <button onclick="berikanRating({{ $item->id }})"
                                                class="inline-flex items-center px-3 py-2 border border-blue-300 text-sm font-medium rounded-md text-blue-600 bg-white hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('chat.create', $item->id) }}"
                                            class="text-blue-600 hover:text-blue-900 border border-blue-300 rounded-md px-2 py-1 hover:bg-blue-50 transition inline-flex items-center">
                ...
                <div class="p-6 mt-6 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                    <div class="text-sm text-gray-500">
                        Menampilkan {{ $riwayatKonsultasi->firstItem() ?? 0 }}-{{ $riwayatKonsultasi->lastItem() ?? 0 }} dari
                        {{ $riwayatKonsultasi->total() }} hasil
                    </div>
                    <div class="flex flex-wrap justify-center md:justify-end gap-2">
                        ...
                        @else
                            <a href="{{ $riwayatKonsultasi->previousPageUrl() }}"
                                class="px-3 py-1 border rounded-md bg-white text-gray-700 hover:bg-gray-50 flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7">
                                    </path>
                                </svg>
                        ...
                        <div class="flex">
                            @foreach($riwayatKonsultasi->getUrlRange(max($riwayatKonsultasi->currentPage() - 2, 1), min($riwayatKonsultasi->currentPage() + 2, $riwayatKonsultasi->lastPage())) as $page => $url)
                                <a href="{{ $url }}"
                                    class="px-3 py-1 border-t border-b border-r first:border-l {{ $page == $riwayatKonsultasi->currentPage() ? 'bg-blue-50 text-blue-600 font-medium border-blue-300' : 'bg-white text-gray-700 hover:bg-gray-50 border-gray-300' }}">
                                    {{ $page }}
                                </a>
                            @endforeach
                        </div>
                        ...
                        @if($riwayatKonsultasi->hasMorePages())
                            <a href="{{ $riwayatKonsultasi->nextPageUrl() }}"
                                class="px-3 py-1 border rounded-md bg-white text-gray-700 hover:bg-gray-50 flex items-center">
                                Berikutnya
            ...
            <div class="mt-3">
                ...
                <form id="formRating" method="POST" action="">
                    ...
                    <div class="mt-4">
                            ...
                            <div class="flex justify-center space-x-2">
                                ...
                                @for($i = 1; $i <= 5; $i++)
                                    <button type="button" onclick="setRating({{ $i }})"
                                        class="rating-star w-10 h-10 focus:outline-none">
                                        <svg class="w-full h-full text-gray-300" id="star-{{ $i }}" fill="currentColor"
                                            viewBox="0 0 20 20">
                                            <path
                                                d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z">
                                            </path>
                        ...
                        <div class="mb-4">
                            <label for="komentar" class="block text-sm font-medium text-gray-700 mb-1">Komentar
                                (opsional)</label>
                            <textarea id="komentar" name="komentar" rows="3"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Berikan komentar tentang pengalaman konsultasi Anda..."></textarea>
                        </div>
                        <div class="flex justify-end">
                            <button type="button" onclick="hideRatingModal()"
                                class="mr-2 px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                Batal
                            </button>
                            <button type="submit"
