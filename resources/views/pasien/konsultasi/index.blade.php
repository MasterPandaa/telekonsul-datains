...
@section('pasien-content')
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 flex items-center">
                    <span
                        class="bg-gradient-to-r from-indigo-600 to-blue-500 bg-clip-text text-transparent">Telekonsultasi</span>
                    <span
                        class="ml-3 px-3 py-1 bg-gradient-to-r from-indigo-100 to-blue-100 text-indigo-800 text-xs rounded-full font-medium">Real-time
                        Chat</span>
                </h1>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-2">
                        {{ $konsultasiAktif->where('status', 'Menunggu')->count() + $konsultasiAktif->where('status', 'Terkonfirmasi')->count() }}
                    </p>
                </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden mb-6">
        <div class="border-b">
            <nav class="flex">
                <button class="tab-button px-6 py-4 text-sm font-medium text-blue-600 border-b-2 border-blue-600"
                    data-tab="active">
                    Konsultasi Aktif
                </button>
                <button class="tab-button px-6 py-4 text-sm font-medium text-gray-600 hover:text-gray-800"
                    data-tab="inactive">
                                            ...
                                            @if($item->status === 'Terkonfirmasi' || $item->status === 'Berlangsung')
                                                ...
                                                @if($isWaktuKonsultasi)
                                                    <a href="{{ route('chat.create', $item->id) }}"
                                                        class="bg-green-600 hover:bg-green-700 text-white rounded-md px-2 py-1 text-center transition">
                                                ...
                                                @else
                                                    <div
                                                        class="text-gray-600 rounded-md px-2 py-1 border border-gray-300 bg-gray-50 flex items-center">
                                                        <svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        ...
                                                        <span class="countdown-timer"
                                                            data-target="{{ $jadwalMulai->timestamp * 1000 }}">Tersisa <span
                                                                class="time-remaining">--:--:--</span></span>
                                                    </div>
                                                @endif
                                            @elseif($item->status === 'Menunggu')
                                                <form action="{{ route('pasien.konsultasi.batalkan', $item->id) }}" method="POST"
                                                    class="inline" id="form-batalkan-{{ $item->id }}">
                                                    @csrf
                                                    <button type="button"
                                                        class="text-red-600 hover:text-red-900 border border-red-300 rounded-md px-2 py-1 text-center hover:bg-red-50 transition"
                                                        onclick="konfirmasiBatalkan({{ $item->id }}, '{{ $item->tanggal->isoFormat('D MMMM Y') }}', '{{ substr($item->jam_mulai, 0, 5) }} - {{ substr($item->jam_selesai, 0, 5) }}')">
                                                        <svg class="w-4 h-4 inline-block mr-1" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div id="tab-inactive" class="bg-white rounded-lg shadow-md overflow-hidden mb-6 hidden">
        ...
        @php
            $konsultasiTidakAktif = $riwayatKonsultasi->filter(function ($item) {
                return in_array($item->status, ['Dibatalkan', 'Terlambat', 'Ditolak']);
            });
        @endphp
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <img class="h-10 w-10 rounded-full object-cover bg-gray-100"
                                                    src="{{ $item->dokter->dokter->foto_url ?? asset('img/BW_ASSRI.png') }}"
                                                    alt="{{ $item->dokter->name ?? 'Dokter' }}">
                                            </div>
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $item->dokter->name ?? '-' }}
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex flex-col sm:flex-row gap-2">
                                            ...
                                            <button type="button" onclick="hapusKonsultasi({{ $item->id }})"
                                                class="text-red-600 hover:text-red-900 border border-red-300 rounded-md px-2 py-1 hover:bg-red-50 transition inline-flex items-center">
                                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
