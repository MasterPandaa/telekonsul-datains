...
@php
    $konsultasiAktifFiltered = array_filter($konsultasiAktif ?? [], function($item) {
        return in_array($item['status'], ['Terkonfirmasi', 'Menunggu', 'Berlangsung']);
    });
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-10 w-10">
                                <img class="h-10 w-10 rounded-full object-cover bg-gray-100" src="{{ $item['pasien_foto_url'] ?? \App\Support\ProfilePhoto::getDefaultUrl() }}" alt="{{ $item['pasien_nama'] }}">
                            </div>
                            <div class="ml-4">
                    ...
                    <td class="px-6 py-4 text-center">
                        @if($item['status'] == 'Terkonfirmasi' || $item['status'] == 'Berlangsung')
                            @php
                                ...
                                $startTime = \Carbon\Carbon::parse($item['tanggal_tampil']->format('Y-m-d') . ' ' . $item['jam_mulai']);
                                $isTimeToStart = $now->gte($startTime);
                            @endphp
                            ...
                            @else
                                <div class="rounded inline-flex items-center justify-center">
                                    <div class="text-center">
                                        <span class="countdown-timer inline-flex items-center text-sm text-gray-600 rounded-lg px-3 py-1.5 border border-gray-200 bg-gray-50 whitespace-nowrap" data-target="{{ $startTime->timestamp * 1000 }}">
        ...
        <div class="flex items-center justify-between">
            <div class="flex-1 flex justify-between sm:hidden">
                @if($konsultasiAktifPaginator && $konsultasiAktifPaginator->total() > 0)
                    <a href="{{ $konsultasiAktifPaginator->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 {{ $konsultasiAktifPaginator->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                        Sebelumnya
                    </a>
                    <a href="{{ $konsultasiAktifPaginator->nextPageUrl() }}" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 {{ !$konsultasiAktifPaginator->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}">
                        Selanjutnya
                    </a>
                @endif
            </div>
                ...
                @if($konsultasiAktifPaginator && $konsultasiAktifPaginator->total() > 0)
                    <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            <a href="{{ $konsultasiAktifPaginator->previousPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 {{ $konsultasiAktifPaginator->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                            ...
                            <a href="#" aria-current="page" class="z-10 bg-blue-50 border-blue-500 text-blue-600 relative inline-flex items-center px-4 py-2 border text-sm font-medium">
                                {{ $konsultasiAktifPaginator->currentPage() }}
                            </a>
                            <a href="{{ $konsultasiAktifPaginator->nextPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 {{ !$konsultasiAktifPaginator->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}">
        ...
        <div class="flex items-center justify-between">
            <div class="flex-1 flex justify-between sm:hidden">
                @if($konsultasiTidakAktifPaginator && $konsultasiTidakAktifPaginator->total() > 0)
                    <a href="{{ $konsultasiTidakAktifPaginator->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 {{ $konsultasiTidakAktifPaginator->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                        Sebelumnya
                    </a>
                    <a href="{{ $konsultasiTidakAktifPaginator->nextPageUrl() }}" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 {{ !$konsultasiTidakAktifPaginator->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}">
                        Selanjutnya
                    </a>
                @endif
            </div>
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                ...
                @if($konsultasiTidakAktifPaginator && $konsultasiTidakAktifPaginator->total() > 0)
                    <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            <a href="{{ $konsultasiTidakAktifPaginator->previousPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 {{ $konsultasiTidakAktifPaginator->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                                <span class="sr-only">Sebelumnya</span>
                                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                                </svg>
                            ...
                            <a href="#" aria-current="page" class="z-10 bg-blue-50 border-blue-500 text-blue-600 relative inline-flex items-center px-4 py-2 border text-sm font-medium">
                                {{ $konsultasiTidakAktifPaginator->currentPage() }}
                            </a>
                            <a href="{{ $konsultasiTidakAktifPaginator->nextPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 {{ !$konsultasiTidakAktifPaginator->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}">
                                <span class="sr-only">Selanjutnya</span>
                                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                </svg>
...
function konfirmasiTerima(id) {
    Swal.fire({
        ...
        text: 'Apakah Anda yakin ingin menerima permintaan konsultasi ini?',
        icon: 'question',
        ...
        if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = `/dokter/konsultasi/${id}/konfirmasi`;
...
function konfirmasiTolak(id) {
    Swal.fire({
        ...
        input: 'text',
        inputLabel: 'Masukkan alasan penolakan konsultasi',
        ...
        if (result.isConfirmed) {
            ...
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/dokter/konsultasi/${id}/tolak`;
...
function hapusKonsultasi(id) {
    Swal.fire({
        ...
        text: 'Apakah Anda yakin ingin menghapus konsultasi ini dari riwayat?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d33',
        cancelButtonColor: '#3085d6',
        ...
        if (result.isConfirmed) {
            ...
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/dokter/konsultasi/${id}/hapus`;
