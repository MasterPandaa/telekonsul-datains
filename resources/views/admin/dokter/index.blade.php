...
<div class="mb-6">
    ...
    <p class="text-sm text-gray-600">Kelola data dokter pada sistem</p>
</div>
...
<div class="bg-white rounded-lg shadow-md overflow-hidden">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between p-4 border-b">
        <h2 class="text-lg font-medium text-gray-800 mb-3 md:mb-0">Daftar Dokter</h2>
        
        <div class="flex flex-col md:flex-row items-start md:items-center space-y-3 md:space-y-0 md:space-x-3 w-full md:w-auto">
            <!-- Filter and Search -->
            <form action="{{ route('admin.dokter.index') }}" method="GET" class="flex items-center w-full md:w-auto">
                <div class="relative flex-grow">
                    <input type="text" name="search" placeholder="Cari dokter..." value="{{ $searchTerm ?? '' }}" 
                ...
                @if(!empty($searchTerm))
                    <a href="{{ route('admin.dokter.index') }}" class="ml-2 text-sm text-gray-600 hover:text-gray-900">
            ...
            <a href="{{ route('admin.dokter.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition">
                ...
                <tr class="bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    ...
                    <th class="px-4 py-3 border-b">
                        <a href="{{ route('admin.dokter.index', [
                            'search' => $searchTerm ?? '', 
                            'sort_by' => 'no_sip', 
                            'sort_order' => $sortBy === 'no_sip' && $sortOrder === 'asc' ? 'desc' : 'asc'
                        ]) }}" class="flex items-center group">
                            <span>No. SIP</span>
                            <span class="ml-1">
                                @if($sortBy === 'no_sip')
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <div class="text-sm text-gray-500">{{ $dokter->jenis_kelamin }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-sm text-center">
                        <div class="flex items-center justify-center space-x-2">
                            <!-- Lihat Detail -->
                            <a href="{{ route('admin.dokter.show', $dokter->id) }}" class="p-1.5 bg-blue-100 text-blue-600 rounded-md hover:bg-blue-200 transition">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- Edit -->
                            <a href="{{ route('admin.dokter.edit', $dokter->id) }}" class="p-1.5 bg-yellow-100 text-yellow-600 rounded-md hover:bg-yellow-200 transition">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- Hapus -->
                            <button type="button" 
                                    onclick="konfirmasiHapus('{{ $dokter->id }}', '{{ $dokter->nama }}')" 
                                    class="p-1.5 bg-red-100 text-red-600 rounded-md hover:bg-red-200 transition">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                </svg>
                            </button>
                            <form id="form-hapus-{{ $dokter->id }}" action="{{ route('admin.dokter.destroy', $dokter->id) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                        </div>
                    </td>
                ...
                <tr>
                    <td colspan="5" class="px-4 py-3 text-center text-gray-500">
                        <div class="flex flex-col items-center justify-center py-6">
                            ...
                            <p class="text-gray-500">Tidak ada data dokter yang tersedia.</p>
                            <a href="{{ route('admin.dokter.create') }}" class="mt-2 text-sm text-blue-600 hover:text-blue-800">
                                Tambahkan data dokter baru
                            </a>
                        </div>
                    </td>
                </tr>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    ...
                    @else
                        <p class="text-sm text-gray-500 italic">Belum ada data dokter saat ini</p>
                    @endif
                </div>
                @if($dokters->count() > 0)
                    <div>
