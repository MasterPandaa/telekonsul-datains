...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <p class="text-sm text-gray-600">Informasi lengkap data dosen</p>
    </div>
    <div class="flex space-x-3">
        <a href="{{ route('admin.dosen.edit', $dosen) }}" class="inline-flex items-center px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition">
    ...
    <div class="md:flex">
        ...
        <div class="md:w-1/3 bg-blue-50 p-6 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-gray-200">
            <div class="w-32 h-32 rounded-full bg-blue-100 mb-4 overflow-hidden">
                <img src="https://ui-avatars.com/api/?name={{ urlencode($dosen->nama) }}&background=3b82f6&color=fff&size=128" alt="{{ $dosen->nama }}" class="w-full h-full object-cover">
            </div>
            
            <h2 class="text-xl font-bold text-gray-800 text-center">{{ $dosen->nama }}</h2>
            <p class="text-blue-600 font-medium mb-4">{{ $dosen->nip }}</p>
            ...
            <div class="w-full mt-4 space-y-3">
                ...
                <div class="flex items-center justify-center text-sm">
                    ...
                    <span>{{ $dosen->no_hp ?? 'Belum diisi' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="md:w-2/3 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-gray-800">{{ $dosen->nama }}</p>
                </div>
            ...
            <div class="mt-8 pt-4 border-t">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span class="text-gray-700 ml-2">{{ $dosen->updated_at->diffForHumans() }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">Didaftarkan pada:</span>
                        <span class="text-gray-700 ml-2">{{ $dosen->created_at->format('d M Y H:i') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden border-l-4 border-red-500">
        <div class="p-6">
            <div class="flex items-center justify-between">
                <div>
                    <h4 class="font-medium text-gray-800">Hapus Data Dosen</h4>
