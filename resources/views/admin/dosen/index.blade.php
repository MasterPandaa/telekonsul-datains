    ...
    <div class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
        <div class="flex items-center justify-between">
            <div class="flex-1 flex justify-between sm:hidden">
                @if($dosens->count() > 0)
                    <a href="{{ $dosens->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 {{ $dosens->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                        Sebelumnya
                    </a>
                    <a href="{{ $dosens->nextPageUrl() }}" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 {{ !$dosens->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}">
                        Selanjutnya
                    </a>
                @endif
            </div>
                ...
                <div>
                    @if($dosens->count() > 0)
                        <p class="text-sm text-gray-700">
                            Menampilkan <span class="font-medium">{{ $dosens->firstItem() }}</span> sampai <span class="font-medium">{{ $dosens->lastItem() }}</span> dari <span class="font-medium">{{ $dosens->total() }}</span> hasil
                        </p>
                    @else
                        <p class="text-sm text-gray-500 italic">Belum ada data dosen saat ini</p>
                    @endif
                </div>
                @if($dosens->count() > 0)
                    <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            <a href="{{ $dosens->previousPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 {{ $dosens->onFirstPage() ? 'opacity-50 cursor-not-allowed' : '' }}">
                            ...
                            <a href="{{ $dosens->nextPageUrl() }}" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 {{ !$dosens->hasMorePages() ? 'opacity-50 cursor-not-allowed' : '' }}">
